...
| **User photos/video** | Vercel Blob (`BLOB_READ_WRITE_TOKEN`, auto-injected) | Direct presigned PUT from the browser. R2 supported as an alternative. |
...
### User uploads (photos and video)

Live, backed by **Vercel Blob**. The store was provisioned with
`vercel blob create-store pour-finder-media --access public`, which injects
`BLOB_READ_WRITE_TOKEN` into the project automatically — no credential is ever
copied by hand.

The browser uploads **directly** to storage via a short-lived presigned PUT.
Routing bytes through the app would burn serverless bandwidth and hit Vercel's
~4.5 MB request-body limit, which rules out video entirely.

| | Photo | Video |
|---|---|---|
| Types | JPEG, PNG, WebP, HEIC | MP4, MOV, WebM |
| Max size | 8 MB | 40 MB |
| Max length | — | 30 s |
...
egress fees**, where Vercel Blob's free tier is ~1 GB with metered bandwidth.
The storage layer already supports both and prefers R2 when configured.

1. Enable R2 in the Cloudflare dashboard (needs accepting R2's terms and a card
   on file, even for the free tier).
2. Create a bucket and an S3 API token.
3. Set `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`,
   `R2_BUCKET`, `R2_PUBLIC_BASE_URL`.
4. Add a CORS rule on the bucket allowing `PUT` from your site origin —
   browser uploads fail without it. Vercel Blob needs no equivalent.

Existing Blob objects keep working: each row records the provider and its
resolved URL, so the two coexist and only new uploads go to R2.
